...
def clockwise(id):
    ...
    while True:
        ...
        if ids is not None and [id] in ids:
            ...
            if np.max(mask_blue) - np.min(mask_blue) < 20:
                motors.command(70, Direction.FORWARD)
                time.sleep(0.3)
            elif np.abs(np.min(cols) - np.min(cb)) < 10:
                motors.command(90, Direction.RIGHT)
                time.sleep(1)
                motors.command(75, Direction.FORWARD)
                time.sleep(1)
                motors.command(90, Direction.LEFT)
                time.sleep(0.3)
            elif np.abs(np.max(cols) - np.max(cb)) < 10:
                motors.command(90, Direction.LEFT)
                time.sleep(1)
                motors.command(75, Direction.FORWARD)
                time.sleep(1)
